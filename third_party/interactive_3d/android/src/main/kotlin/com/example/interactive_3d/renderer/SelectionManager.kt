...
internal class SelectionManager {
    ...
    private fun applyOverrideParamsToInstance(mat: MaterialInstance, params: Map<String, Any>) {
        (params["color"] as? List<Double>)?.takeIf { it.size == 4 }?.let { c ->
            // Match the selection/cache path: use the requested override color
            // as the sole color source. The app's anatomy GLB uses very dark
            // source materials, so tinting the base material can remain nearly
            // black on Android/Filament.
            mat.setParameter("baseColorIndex", -1)
            mat.setParameter("metallicRoughnessIndex", -1)
            mat.setParameter("normalIndex", -1)
            mat.setParameter("emissiveIndex", -1)
            mat.setParameter(
                "baseColorFactor",
